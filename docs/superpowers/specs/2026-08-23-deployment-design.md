...
| T7  | Malicious or careless dependency                        | `npm ci` against the lockfile, `npm audit` at high and blocking, SBOM per image, CodeQL weekly. Dependabot was removed (2026-08-30): its first run produced ten major-version pull requests, none of them wanted, and the blocking audit already fails the build on a vulnerable dependency |
...
| T17 | **A fork's pull request reaches a privileged workflow** | `build.yml` requires `workflow_run.event == 'push'` and that the triggering run came from this repository; `deploy.yml` requires the same identity. See §9.2 — reachable only once the repository became public                                                                             |

### 9.2 The `workflow_run` hole, and why the branch filter was not enough

Worth writing out, because the first version of this design had it wrong and the mistake is
an easy one to repeat.

`workflow_run` jobs run in the **base repository's** context — its secrets, its permissions.
A `pull_request` CI run, by contrast, runs in the **fork's** context and can modify `ci.yml`
itself. The `branches: [main]` filter looked like it closed the gap. It does not: it filters
on `workflow_run.head_branch`, which is a name the fork chooses, and a fork whose branch is
called `main` passes it.

The chain that opened the moment the repository went public:

1. Fork the repository, name the branch `main`, open a pull request.
2. CI runs unprivileged in the fork — but its completion fires `workflow_run`.
3. `build.yml` runs **privileged**, with `packages: write`, checking out
   `workflow_run.head_sha` — the fork's code — and publishing it as `:latest`.
4. Build succeeding chains to `deploy.yml`, which holds `SSH_KEY`.

Two clauses close it. `workflow_run.event == 'push'` works because a fork cannot cause a
push event on this repository, and a fork's pull request always reports `pull_request`.
`head_repository.full_name == github.repository` says the same thing more directly.

**The two clauses are not interchangeable between the workflows.** In `deploy.yml` the
`workflow_run` object is the _Build_ run, and Build is itself triggered by `workflow_run`,
so its `.event` is `workflow_run` and never `push`. Requiring `event == 'push'` there would
mean no deployment ever fires again. Only the identity check transfers.
